...
export class CachedGithubApiService {
    ...
    public getUserRepos(username: string, pageSize = 25, page?: number): Observable<UserRepositoriesResponse> {
        return this.api.getUserRepos(username, pageSize, page).pipe(
            mergeMap((value: UserRepositoriesResponse): Observable<UserRepositoriesResponse> => {
                return from(this.cacheService.put(value)).pipe(
                    map((): UserRepositoriesResponse => {
                        return value;
                    })
                );
            })
